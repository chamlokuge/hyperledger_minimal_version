...
addCapabilityToChannel() {
        ...
        # Get the current channel config, decode and write it to 
        fetchChannelConfig $CH_NAME 

        # Modify the correct section of the config based on capabilities group
        if [ $GROUP == "orderer" ]; then
                jq -s '.[0] * {"channel_group":{"groups":{"Orderer": {"values": {"Capabilities": .[1].orderer}}}}}'  ./scripts/capabilities.json > modified_
        elif [ $GROUP == "channel" ]; then
                jq -s '.[0] * {"channel_group":{"values": {"Capabilities": .[1].channel}}}'  ./scripts/capabilities.json > modified_
        elif [ $GROUP == "application" ]; then
                jq -s '.[0] * {"channel_group":{"groups":{"Application": {"values": {"Capabilities": .[1].application}}}}}'  ./scripts/capabilities.json > modified_
        fi

        # Create a config updated for this channel based on the differences between  and modified_
        # write the output to config_update_in_envelope.pb
        createConfigUpdate "$CH_NAME"  modified_ config_update_in_envelope.pb
